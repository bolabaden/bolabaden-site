...
function StatLink({ href, icon: Icon, label, title }: {
  href: string
...
function calculateLanguagePercentages(languages: { [lang: string]: number }): Array<{ name: string; percentage: number; bytes: number }> {
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          {githubStats?.isFork && (
            <div className="flex items-center gap-1 text-xs bg-amber-500/20 text-amber-300 px-2 py-1 rounded" title="This is a forked repository">
              <GitBranch className="h-3 w-3" />
              Fork
            </div>
          )}
          {githubStats?.isArchived && (
            <div className="flex items-center gap-1 text-xs bg-gray-500/20 text-gray-300 px-2 py-1 rounded" title="This repository is archived">
      ...
      <AnimatePresence>
        {showCommitGraph && githubStats?.commitActivity && githubStats.commitActivity.length > 0 && (
          ...
          >
            ...
            <CommitGraph data={githubStats.commitActivity} repoUrl={project.githubUrl} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {githubStats?.languages && Object.keys(githubStats.languages).length > 0 && (
        <div className="mb-4">
          <div className="text-xs text-muted-foreground/80 mb-2 font-medium">Languages</div>
          <div className="space-y-2">
            {calculateLanguagePercentages(githubStats.languages).slice(0, 5).map((lang) => (
      ...
      {githubStats?.topContributors && githubStats.topContributors.length > 0 && project.githubUrl && (
        <div className="mb-4">
          <Link
            href={`${project.githubUrl}/graphs/contributors`}
            target="_blank"
