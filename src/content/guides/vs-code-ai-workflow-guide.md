...
### Mitigating AI Sycophancy and Conversational Filler
...
The phenomenon of an AI appending elements of the original prompt to its output is classified under several known behaviors:

- **Sycophancy (Prompt Echoing):** The model attempts to flatter or overly affirm you by echoing your input (e.g., "Yes, you are correct," or "As you requested...").
- **Conversational Preambles/Postambles:** Structural filler where the AI surrounds its technical payload with human-like conversational bookends (e.g., "Certainly! I'd be happy to help with...", "Let me know if you need anything else!").
- **Prompt Reflection / Embedded Meta-Commentary:** When the model embeds its own internal reasoning or instructions directly into the output design itself (e.g., explaining *why* it made an edit instead of just making it).

#### System Prompt Prevention Rules

The most effective, scalable way to prevent these behaviors autonomously is by injecting negative constraints at the system level. These instructions operate above the user's session context and preemptively neutralize the AI's default conversational behaviors.

To enforce this discipline in VS Code, append the following constraints to an agent configuration file (e.g., `.github/copilot-instructions.md`, `agents.md`, or a project's standard `system-prompt.txt`). Other prompt engineers and developers have reported these specific negative commands work extremely well:
...
2. **Audit Systemic Affirmations:** Search for and strip out common hedging or affirmative phrases (e.g., "It's important to note", "Crucially").
3. **Neutralize Subjectivity:** LLMs default to using hyperbolic adjectives to describe technical implementations (e.g., "revolutionary", "flawlessly"). Replace these with purely objective, factual descriptions.
4. **Remove Self-Referential Explanations:** Delete any text where the documentation justifies its own existence or references the fact that an AI generated it.
