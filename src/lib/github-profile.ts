/**
 * Comprehensive GitHub profile statistics and analytics.
 *
 * CONTEXT: Portfolio/Flex-Focused Profile Evidence
 * Aggregates PRs, issues, reviews, contributions calendar, language proficiency across all repos.
 * Supports GitHubStatsSection (/about) narrative showing breadth and depth of GitHub presence.
 * Powers portfolio evidence of collaboration, contributions, and multi-repo expertise.
 */
...
export async function fetchProfile(
  ...
  return ghFetch<GitHubUserProfile>(
...
export async function buildActivitySummary(
  ...
  for (const e of events) {
    ...
    if (e.repo?.name)
      repoCounts[e.repo.name] = (repoCounts[e.repo.name] || 0) + 1;
    if (e.created_at) activeDaySet.add(e.created_at.split("T")[0]);
  }

  const mostActiveRepo =
    Object.keys(repoCounts).sort((a, b) => repoCounts[b] - repoCounts[a])[0] ??
    null;
...
export async function fetchReviewStats(
  username: string,
): Promise<GitHubReviewStats> {
  const reviews = await searchIssues(
...
export async function buildRepoSummary(
  ...
  return {
    ...
    mostStarredRepo: sortedByStars[0]
      ? {
          name: sortedByStars[0].full_name,
    ...
    mostForkedRepo: sortedByForks[0]
      ? {
          name: sortedByForks[0].full_name,
          forks: sortedByForks[0].forks_count,
          url: sortedByForks[0].html_url,
        }
      : null,
    languageBreakdown: languageTotals,
    topLanguages,
    openSourceRepos: repos.filter((r) => !r.fork && !r.archived && !r.private)
      .length,
...
export async function fetchComprehensiveStats(
  username: string,
): Promise<GitHubComprehensiveStats | null> {
  try {
