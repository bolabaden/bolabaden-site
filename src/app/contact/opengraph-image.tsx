  ...
  return new ImageResponse(
    <div
      style={{
        ...
        background:
          "linear-gradient(135deg, hsl(222 47% 11%) 0%, hsl(217 32% 17%) 64%, hsl(217 91% 60% / 0.25) 100%)",
        color: "hsl(210 20% 98%)",
        padding: "62px",
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        <div style={{ fontSize: 62, fontWeight: 700 }}>
          {config.OG_CONTACT_TITLE}
        </div>
        <div
          style={{ fontSize: 34, color: "hsl(217 91% 72%)", fontWeight: 600 }}
        >
          {config.OG_CONTACT_SUBTITLE}
        </div>
        <div style={{ fontSize: 26, opacity: 0.92, maxWidth: "1000px" }}>
          {config.OG_CONTACT_DESCRIPTION}
        </div>
      </div>
      <div style={{ fontSize: 22, opacity: 0.8 }}>{config.CONTACT_EMAIL}</div>
    </div>,
    size,
  );
}
