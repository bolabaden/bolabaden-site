 ...
 * Fetches static projects enriched with realtime GitHub stats and enhanced metadata.
 *
 * CONTEXT: Generic/Reference Data with Enhancement
 * Returns curated project list with calculated stats (activity, language proficiency)
 * for presentation in cards and detailed views.
 * Used by both portfolio and discovery contexts for rich project data.
 */
  ...
  try {
    ...
    if (category && category !== "all") {
      filteredProjects = filteredProjects.filter(
        (ep) => ep.project.category === category,
      );
    }
