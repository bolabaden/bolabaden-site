...
/**
 * GET /api/containers
 * Fetches safe-to-embed Docker containers from host infrastructure.
 *
 * CONTEXT: Portfolio/Flex-Focused Infrastructure Evidence
 * Returns containers marked with Traefik routing labels (publicly accessible).
 * Used to enumerate live services in portfolio context for discovery and status.
 * Only includes read-only safe services (Prometheus, Grafana, SearXNG, etc).
 * Sensitive infrastructure containers are filtered out.
 */
  ...
  try {
    ...
    const safeEmbeds = containers
      ...
      .map((container: any) => {
        ...
        const labels = container.Labels || {};
        const description =
          labels["homepage.description"] || `${container.Image} service`;
